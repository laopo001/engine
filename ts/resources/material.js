pc.extend(pc, (() => {
    ...
    const onCubemapAssetLoad = function({resources}, attribute, newValue, oldValue) {
        const props = [
            'cubeMap',
    ...
    class MaterialHandler {
        constructor({assets, graphicsDevice}) {
            this._assets = assets;
        ...
        _createPlaceholders() {
            ...
            for (const key in textures) {
                ...
                // create texture
                const texture = placeholders[key] = new pc.Texture(this._device, {
                    width: 2,
                    height: 2,
        ...
        _updateStandardMaterial(asset, data, assets) {
            ...
            const pathMapping = (data.mapping_format === "path");
            ...
            data.parameters.forEach((param, i) => {
                ...
                if (param.type === 'texture') {
                    ...
                    // asset handler
                    let handler = material._assetHandlers[param.name];
                    ...
                    if (param.data && !(param.data instanceof pc.Texture)) {
                        ...
                        if (asset) {
                            if (asset.resource) {
                                ...
                                const texture = placeholders[placeholdersMapping[data.parameters[i].name]];
                                if (texture) {
                    ...
                    if (asset) {
                        ...
                        assets.once(`add:url:${pc.path.join(dir, param.data)}`, asset => {
                            asset.ready(asset => {
                                // TODO
                                // update specific param instead of all of them
