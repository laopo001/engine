pc.extend(pc, (() => {
     ...
     */
    const Batch = function (meshInstances, dynamic, batchGroupId) {
     ...
     */
    const BatchGroup = function (id, name, dynamic, maxAabbSize, layers) {
        this.dynamic = dynamic;
    ...
    class BatchManager {
        ...
        _collectAndRemoveModels(node, groupMeshInstances, groupIds) {
            if (!node.enabled) return;

            let i;
            if (node.model && node.model.batchGroupId >= 0 && node.model.model && node.model.enabled) {
                if (!groupIds || (groupIds && groupIds.includes(node.model.batchGroupId))) {
                    ...
                    if (node.model.isStatic) {
                        ...
                        for (i=0; i<drawCalls.length; i++) {
                            ...
                            if (!nodeMeshInstances.includes(drawCalls[i]._staticSource)) continue;
                            groupMeshInstances[node.model.batchGroupId].push(drawCalls[i]);
                        }
                        for (i=0; i<nodeMeshInstances.length; i++) {
                            if (drawCalls.includes(nodeMeshInstances[i])) {
            ...
            if (node.element && node.element.batchGroupId >= 0 && node.element.enabled) {
                if (!groupIds || (groupIds && groupIds.includes(node.element.batchGroupId))) {
            ...
            if (node.sprite && node.sprite.batchGroupId >= 0 && node.sprite.enabled) {
                if (!groupIds || (groupIds && groupIds.includes(node.sprite.batchGroupId))) {
        ...
        _markGroupDirty(id) {
            if (!this._dirtyGroups.includes(id)) {
                this._dirtyGroups.push(id);
            }
        }

        _registerEntities(batch, meshInstances) {
            let node;
            const ents = [];
        ...
        generate(groupIds) {
            ...
            if (!groupIds) {
                ...
                const newBatchList = [];
                for (i=0; i<this._batchList.length; i++) {
                    if (!groupIds.includes(this._batchList[i].batchGroupId)) {
                        newBatchList.push(this._batchList[i]);
                        continue;
                    }
                    this._batchList[i].refCounter = 1;
                ...
                if (groupIds === this._dirtyGroups) {
                    ...
                    for (i=0; i<this._dirtyGroups.length; i++) {
                        if (!groupIds.includes(this._dirtyGroups[i])) newDirtyGroups.push(this._dirtyGroups[i]);
                    }
        ...
        prepare(meshInstances, dynamic, maxAabbSize) {
            ...
            let i;
            let material, layer, vertCount, params, params2, param, paramFailed, lightList, defs;
            const aabb = new pc.BoundingBox();
            const testAabb = new pc.BoundingBox();
                    ...
                    if (i > 0) {
                        ...
                        if (lightList && params2) {
                            ...
                            for (k=0; k<lightList.length; k++) {
                                if (!params2.includes(lightList[k])) {
                                    paramFailed = true;
                                    break;
                                }
                            }
                            for (k=0; k<params2.length; k++) {
                                if (!lightList.includes(params2[k])) {
                                    paramFailed = true;
                                    break;
                                }
                            }
        ...
        create(meshInstances, dynamic, batchGroupId) {
            ...
            let i, j;
            const batch = new pc.Batch(meshInstances, dynamic, batchGroupId);
            this._batchList.push(batch);

            // Check which vertex format and buffer size are needed, find out material
            let material = null;
            let mesh, elems, numVerts, vertSize;
            let hasPos, hasNormal, hasUv, hasUv2, hasTangent, hasColor;
            let batchNumVerts = 0;
            ...
            // Create buffers
            const entityIndexSizeF = dynamic ? 1 : 0;
            const batchVertSizeF = 3 + (hasNormal ? 3 : 0) + (hasUv ? 2 : 0) +  (hasUv2 ? 2 : 0) + (hasTangent ? 4 : 0) + (hasColor ? 1 : 0) + entityIndexSizeF;
            const batchOffsetNF = 3;
            const batchOffsetUF = hasNormal ? 3*2 : 3;
            const batchOffsetU2F = (hasNormal ? 3*2 : 3) + (hasUv ? 2 : 0);
            const batchOffsetTF = (hasNormal ? 3*2 : 3) + (hasUv ? 2 : 0) + (hasUv2 ? 2 : 0);
            const batchOffsetCF = (hasNormal ? 3*2 : 3) + (hasUv ? 2 : 0) + (hasUv2 ? 2 : 0) + (hasTangent ? 4 : 0);
            const batchOffsetEF = (hasNormal ? 3*2 : 3) + (hasUv ? 2 : 0) + (hasUv2 ? 2 : 0) + (hasTangent ? 4 : 0) + (hasColor ? 1 : 0);

            const arrayBuffer = new ArrayBuffer(batchNumVerts * batchVertSizeF * 4);
            const batchData = new Float32Array(arrayBuffer);
            const batchData8 = new Uint8Array(arrayBuffer);

            const indexBuffer = new pc.IndexBuffer(this.device, pc.INDEXFORMAT_UINT16, batchNumIndices, pc.BUFFER_STATIC);
            const batchIndexData = new Uint16Array(indexBuffer.lock());
            let vertSizeF;

            // Fill vertex/index/matrix buffers
            let data, data8, indexBase, numIndices, indexData;
            let verticesOffset = 0;
            let indexOffset = 0;
            let vbOffset = 0;
            let offsetPF, offsetNF, offsetUF, offsetU2F, offsetTF, offsetCF;
            let transform, vec, vecData;
            if (!dynamic) {
                vec = new pc.Vec3();
                vecData = vec.data;
            }
            ...
            let vertexFormat = this.vertexFormats[vertexFormatId];
            if (!vertexFormat) {
                const formatDesc = [];
                formatDesc.push({
            ...
            // Upload data to GPU
            const vertexBuffer = new pc.VertexBuffer(this.device, vertexFormat, batchNumVerts, pc.BUFFER_STATIC, batchData.buffer);
            indexBuffer.unlock();
            ...
            // Create meshInstance
            const meshInstance = new pc.MeshInstance(this.rootNode, mesh, material);
         ...
         */
        update({_aabb, origMeshInstances, meshInstance}) {
            _aabb.copy(origMeshInstances[0].aabb);
            for (let i=0; i<origMeshInstances.length; i++) {
                if (i > 0) _aabb.add(origMeshInstances[i].aabb); // this is the slowest part
            }
            meshInstance.aabb = _aabb;
            _aabb._radiusVer = -1;
            meshInstance._aabbVer = 0;
        }
         ...
         */
        clone(
            {dynamic, batchGroupId, meshInstance, origMeshInstances},
            clonedMeshInstances
        ) {
            const batch2 = new pc.Batch(clonedMeshInstances, dynamic, batchGroupId);
            ...
            batch2.meshInstance = new pc.MeshInstance(meshInstance.node, meshInstance.mesh, meshInstance.material);
            ...
            batch2.meshInstance.castShadow = meshInstance.castShadow;
            batch2.meshInstance._shader = meshInstance._shader;

            const newModel = new pc.Model();

            newModel.meshInstances = [batch2.meshInstance];
            newModel.castShadows = origMeshInstances[0].castShadows;
        ...
        destroy(batch) {
            ...
            if (batch.refCounter === 0) {
                const layers = this._batchGroups[batch.batchGroupId].layers;
