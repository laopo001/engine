pc.extend(pc, (() => {
    ...
    class LayerComposition {
        ...
        _sortLights({_lights, _sortedLights}) {
            let light;
        ...
        _update() {
            ...
            if (this._dirtyBlend) {
                ...
                let opaqueOld, transparentOld, opaqueNew, transparentNew;
                for (i=0; i<len; i++) {
                    layer = this.layerList[i];
                    if (layer.passThrough) continue;
                ...
                for (i=0; i<len; i++) {
                    ...
                    for (j=0; j<arr.length; j++) {
                        ...
                        for (k=0; k<meshInstances.length; k++) {
                            if (!casters.includes(meshInstances[k])) casters.push(meshInstances[k]);
                        }
                    }
                }
            }
            ...
            if ((result & pc.COMPUPDATED_LIGHTS) || this._dirtyCameras) {
                ...
                const globalLights = this._sortedLights[pc.LIGHTTYPE_DIRECTIONAL];
                for (l=0; l<globalLights.length; l++) {
                    light = globalLights[l];
                    this._globalLightCameras[l] = [];
                    for (i=0; i<len; i++) {
                        layer = this.layerList[i];
                        if (!layer._sortedLights[pc.LIGHTTYPE_DIRECTIONAL].includes(light)) continue;
                        for (k=0; k<layer.cameras.length; k++) {
                            if (this._globalLightCameras[l].includes(layer.cameras[k])) continue;
            ...
            if (this._dirtyCameras) {
                ...
                let hash, hash2, groupLength, cam;
                let skipCount = 0;
