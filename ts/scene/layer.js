pc.extend(pc, (() => {
    ...
    function sortManual({drawOrder}, {drawOrder}) {
        return drawOrder - drawOrder;
    }

    function sortMaterialMesh({_key, mesh}, {_key, mesh}) {
        keyA = _key[pc.SORTKEY_FORWARD];
        keyB = _key[pc.SORTKEY_FORWARD];
    ...
    function sortBackToFront({zdist}, {zdist}) {
        return zdist - zdist;
    }

    function sortFrontToBack({zdist}, {zdist}) {
        return zdist - zdist;
    }

    const sortCallbacks = [null, sortManual, sortMaterialMesh, sortBackToFront, sortFrontToBack];

    function sortCameras({priority}, {priority}) {
        return priority - priority;
    }

    function sortLights({key}, {key}) {
        return key - key;
    }
    ...
    const VisibleInstanceList = function () {
        this.list = [];
        this.length = 0;
        this.done = false;
    };
    ...
    class Layer {
        ...
        addMeshInstances(meshInstances, skipShadowCasters) {
            ...
            for (let i=0; i<meshInstances.length; i++) {
                ...
                if (!skipShadowCasters && m.castShadow && !casters.includes(m)) casters.push(m);
        ...
        removeMeshInstances(meshInstances, skipShadowCasters) {
            ...
            let i, j, m, spliceOffset, spliceCount, len, drawCall;
         ...
         */
        addLight(light) {
            if (this._lightComponents.includes(light)) return;
         ...
         */
        removeLight(light) {
            let id = this._lightComponents.indexOf(light);
            if (id < 0) return;
         ...
         */
        removeCamera(camera) {
            const id = this.cameras.indexOf(camera);
            if (id < 0) return;
