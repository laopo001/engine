namespace pc {
    ...
    export const standard = {
        ...
        createShaderDefinition(device, options) {
            ...
            const codes: [string, string, string, Array<any>] = [code, varyings, codeBody, []];
            ...
            if (options.useSpecular && (lighting || reflections)) {
                ...
                if (options.fresnelModel > 0) {
                    // error
                    if (options.fresnelModel === (pc as any).FRESNEL_SIMPLE) {
                        code += chunks.fresnelSimplePS;
                    } else if (options.fresnelModel === pc.FRESNEL_SCHLICK) {
                        code += chunks.fresnelSchlickPS;
                    // error
                    } else if (options.fresnelModel === (pc as any).FRESNEL_COMPLEX) {
