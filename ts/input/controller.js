    ...
    class Controller {
        ...
        */
        registerAxis(options) {
            ...
            const bind = (controller, source, value, key) => {
                switch (source) {
                    case 'mousex':
                        controller._mouse.on(pc.EVENT_MOUSEMOVE, ({dx}) => {
                            controller._axesValues[name][i] = dx / 10;
                        });
                        break;
                    case 'mousey':
                        controller._mouse.on(pc.EVENT_MOUSEMOVE, ({dy}) => {
                            controller._axesValues[name][i] = dy / 10;
                        });
                        break;
                    case 'key':
                        controller._axes[name].push(() => controller._keyboard.isPressed(key) ? value : 0);
                        break;
                    case 'padrx':
                        controller._axes[name].push(() => controller._gamepads.getAxis(options.pad, pc.PAD_R_STICK_X));
                        break;
                    case 'padry':
                        controller._axes[name].push(() => controller._gamepads.getAxis(options.pad, pc.PAD_R_STICK_Y));
                        break;
                    case 'padlx':
                        controller._axes[name].push(() => controller._gamepads.getAxis(options.pad, pc.PAD_L_STICK_X));
                        break;
                    case 'padly':
                        controller._axes[name].push(() => controller._gamepads.getAxis(options.pad, pc.PAD_L_STICK_Y));
                        break;
                    default:
