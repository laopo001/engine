pc.extend(pc, (function () {
    ...
    var LayerComposition = /** @class */ (function () {
        ...
        LayerComposition.prototype._sortLights = function (_a) {
            var _lights = _a._lights, _sortedLights = _a._sortedLights;
            var light;
            var lights = _lights;
            _sortedLights[pc.LIGHTTYPE_DIRECTIONAL].length = 0;
            _sortedLights[pc.LIGHTTYPE_POINT].length = 0;
            _sortedLights[pc.LIGHTTYPE_SPOT].length = 0;
            for (var i = 0; i < lights.length; i++) {
                light = lights[i];
                if (light._enabled) {
                    _sortedLights[light._type].push(light);
                }
            }
        };
        LayerComposition.prototype._update = function () {
            ...
            if (this._dirty) {
                ...
                for (i = 0; i < len; i++) {
                    ...
                    for (j = 0; j < arr.length; j++) {
                        mi = arr[j];
                        if (!this._meshInstances.includes(mi)) {
            ...
            if (this._dirtyBlend) {
                ...
                var opaqueOld = void 0, transparentOld = void 0, opaqueNew = void 0, transparentNew = void 0;
                for (i = 0; i < len; i++) {
                    layer = this.layerList[i];
                    if (layer.passThrough)
                        continue;
                ...
                for (i = 0; i < len; i++) {
                    ...
                    for (j = 0; j < arr.length; j++) {
                        ...
                        for (k = 0; k < meshInstances.length; k++) {
                            if (!casters.includes(meshInstances[k]))
            ...
            if ((result & pc.COMPUPDATED_LIGHTS) || this._dirtyCameras) {
                ...
                for (l = 0; l < globalLights.length; l++) {
                    ...
                    for (i = 0; i < len; i++) {
                        layer = this.layerList[i];
                        if (!layer._sortedLights[pc.LIGHTTYPE_DIRECTIONAL].includes(light))
                            continue;
                        for (k = 0; k < layer.cameras.length; k++) {
                            if (this._globalLightCameras[l].includes(layer.cameras[k]))
                                continue;
            ...
            if (this._dirtyCameras) {
                ...
                var hash = void 0, hash2 = void 0, groupLength = void 0, cam = void 0;
                var skipCount = 0;
                for (i = 0; i < len; i++) {
                    if (skipCount) {
                        skipCount--;
...
//# sourceMappingURL=layer-composition.js.map