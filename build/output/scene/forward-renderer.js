pc.extend(pc, (function () {
    ...
    function _getZFromAABB(w2sc, aabbMin, aabbMax, lcamMinX, lcamMaxX, lcamMinY, lcamMaxY) {
        ...
        for (var i_1 = 0; i_1 < 8; ++i_1) {
            w2sc.transformPoint(_sceneAABB_LS[i_1], _sceneAABB_LS[i_1]);
        }
        var minz = 9999999999;
        var maxz = -9999999999;
    ...
    function _getZFromAABBSimple(w2sc, aabbMin, aabbMax, lcamMinX, lcamMaxX, lcamMinY, lcamMaxY) {
        ...
        for (var i_2 = 0; i_2 < 8; ++i_2) {
            w2sc.transformPoint(_sceneAABB_LS[i_2], _sceneAABB_LS[i_2]);
            z = _sceneAABB_LS[i_2].z;
            if (z < minz)
                minz = z;
            if (z > maxz)
                maxz = z;
    ...
    //////////////////////////////////////
    function getShadowFormat(_a, shadowType) {
        var webgl2 = _a.webgl2;
    ...
    function getShadowFiltering(_a, shadowType) {
        var webgl2 = _a.webgl2, extTextureFloatLinear = _a.extTextureFloatLinear, extTextureHalfFloatLinear = _a.extTextureHalfFloatLinear;
    ...
    function createShadowCamera(_a, shadowType, type) {
        var webgl2 = _a.webgl2;
        // We don't need to clear the color buffer if we're rendering a depth map
        var flags = pc.CLEARFLAG_DEPTH;
        var hwPcf = shadowType === pc.SHADOW_PCF5 || (shadowType === pc.SHADOW_PCF3 && webgl2);
    ...
    function createShadowBuffer(device, light) {
        ...
        if (light._type === pc.LIGHTTYPE_POINT) {
            if (light._shadowType > pc.SHADOW_PCF3)
                light._shadowType = pc.SHADOW_PCF3; // no VSM or HW PCF point lights yet
    ...
    pc.extend(ForwardRenderer.prototype, {
        ...
        depthSortCompare: function (_a, _b) {
            var _key = _a._key, mesh = _a.mesh;
            var _key = _b._key, mesh = _b.mesh;
        ...
        lightCompare: function (_a, _b) {
            var key = _a.key;
            var key = _b.key;
            return key - key;
        },
        _isVisible: function (_a, _b) {
            var frustum = _a.frustum;
            var visible = _b.visible, aabb = _b.aabb, _aabb = _b._aabb, _aabbVer = _b._aabbVer;
            if (!visible)
                return false;
            meshPos = aabb.center;
        ...
        updateCameraFrustum: function (camera) {
            if (camera.vrDisplay && camera.vrDisplay.presenting) {
                ...
                var parent_1 = camera._node.getParent();
                if (parent_1) {
                    viewMat.copy(parent_1.getWorldTransform()).mul(camera.vrDisplay.combinedViewInv).invert();
                }
                else {
        ...
        setCamera: function (camera, target, clear, cullBorder) {
            ...
            if (!vrDisplay || !vrDisplay.presenting) {
                ...
                if (camera.overrideCalculateTransform) {
                    ...
                    var parent_2 = camera._node.getParent();
                    if (parent_2) {
                        var transform = parent_2.getWorldTransform();
                        ...
                        viewMat.copy(parent_2.getWorldTransform()).mul(vrDisplay.combinedViewInv).invert();
                    }
                    else {
                        // ViewInverse LR
            ...
            device.setScissor(x, y, w, h);
            if (cullBorder)
                device.setScissor(1, 1, pixelWidth - 2, pixelHeight - 2); // optionally clip borders when rendering
        },
        ...
        dispatchDirectLights: function (dirs, _a, mask) {
            var gammaCorrection = _a.gammaCorrection;
        ...
        dispatchPointLight: function (_a, scope, point, cnt) {
            var gammaCorrection = _a.gammaCorrection;
        ...
        dispatchSpotLight: function (_a, scope, spot, cnt) {
            var gammaCorrection = _a.gammaCorrection;
        ...
        cull: function (camera, drawCalls, visibleList) {
            ...
            for (i = 0; i < drawCallsCount; i++) {
                ...
                if (!drawCall.command) {
                    if (!drawCall.visible)
                        continue; // use visible property to quickly hide/show meshInstances
                    visible = true;
                    // if the object's mask AND the camera's cullingMask is zero then the game object will be invisible from the camera
                    if (drawCall.mask && (drawCall.mask & cullingMask) === 0)
                        continue;
            ...
            for (i = 0; i < lights.length; i++) {
                ...
                if (light.castShadows && light._enabled && light.shadowUpdateMode !== pc.SHADOWUPDATE_NONE) {
                    if (type !== pc.LIGHTTYPE_DIRECTIONAL) {
                        ...
                        if (!frustum.containsSphere(tempSphere))
                            continue;
        ...
        setBaseConstants: function (device, _a) {
            var cull = _a.cull, opacityMap = _a.opacityMap, alphaTest = _a.alphaTest;
            // Cull mode
        ...
        setSkinning: function (_a, _b, material) {
            var supportsBoneTextures = _a.supportsBoneTextures;
            var skinInstance = _b.skinInstance;
            if (skinInstance) {
                this._skinDrawCalls++;
        ...
        drawInstance: function (device, meshInstance, _a, style, normal) {
        ...
        // used for stereo
        drawInstance2: function (device, meshInstance, _a, style) {
            var primitive = _a.primitive;
            instancingData = meshInstance.instancingData;
            if (instancingData) {
                this._instancedDrawCalls++;
                ...
                if (light.shadowUpdateMode !== pc.SHADOWUPDATE_NONE && light.visibleThisFrame) {
                    ...
                    if (type === pc.LIGHTTYPE_DIRECTIONAL) {
                        if (light._visibleLength[cameraPass] < 0)
                            continue; // prevent light from rendering more than once for this camera
                    ...
                    while (pass < passes) {
                        ...
                        pass++;
                        if (type === pc.LIGHTTYPE_DIRECTIONAL)
                            light._visibleLength[cameraPass] = -1; // prevent light from rendering more than once for this camera
                    } // end pass
                    if (light._isVsm) {
        ...
        updateShader: function (_a, objDefs, staticLightList, pass, sortedLights) {
            var material = _a.material, _shader = _a._shader;
        ...
        renderForward: function (camera, drawCalls, drawCallsCount, sortedLights, pass, cullingMask, drawCallback, layer) {
            ...
            for (i = 0; i < drawCallsCount; i++) {
                ...
                if (cullingMask && drawCall.mask && !(cullingMask & drawCall.mask))
                    continue; // apply visibility override
                if (drawCall.command) {
                    // We have a command
                    drawCall.command();
                }
                    ...
                    if (camera === pc.skipRenderCamera) {
                        if (pc._skipRenderCounter >= pc.skipRenderAfter)
                            continue;
        ...
        beginFrame: function (_a) {
            var _meshInstances = _a._meshInstances, _lights = _a._lights;
            var device = this.device;
            var scene = this.scene;
            var meshInstances = _meshInstances;
            var lights = _lights;
...
//# sourceMappingURL=forward-renderer.js.map