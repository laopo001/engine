pc.extend(pc, (function () {
    ...
    function sortManual(_a, _b) {
        var drawOrder = _a.drawOrder;
        var drawOrder = _b.drawOrder;
        return drawOrder - drawOrder;
    }
    function sortMaterialMesh(_a, _b) {
    ...
    function sortBackToFront(_a, _b) {
        var zdist = _a.zdist;
        var zdist = _b.zdist;
        return zdist - zdist;
    }
    function sortFrontToBack(_a, _b) {
        var zdist = _a.zdist;
        var zdist = _b.zdist;
        return zdist - zdist;
    }
    var sortCallbacks = [null, sortManual, sortMaterialMesh, sortBackToFront, sortFrontToBack];
    function sortCameras(_a, _b) {
        var priority = _a.priority;
        var priority = _b.priority;
        return priority - priority;
    }
    function sortLights(_a, _b) {
        var key = _a.key;
        var key = _b.key;
        return key - key;
    }
    ...
    var Layer = /** @class */ (function () {
        ...
        Layer.prototype.addMeshInstances = function (meshInstances, skipShadowCasters) {
            ...
            for (var i = 0; i < meshInstances.length; i++) {
                ...
                if (!arr.includes(m))
                    arr.push(m);
                if (!skipShadowCasters && m.castShadow && !casters.includes(m))
                    casters.push(m);
        ...
        Layer.prototype.addLight = function (light) {
            if (this._lightComponents.includes(light))
                return;
        ...
        Layer.prototype._calculateSortDistances = function (drawCalls, drawCallsCount, camPos, camFwd) {
            ...
            for (i = 0; i < drawCallsCount; i++) {
                ...
                if (drawCall.layer <= pc.scene.LAYER_FX)
                    continue; // Only alpha sort mesh instances in the main world (backwards comp)
