pc.extend(pc, (function () {
    ...
    var SkinBatchInstance = /** @class */ (function () {
        ...
        SkinBatchInstance.prototype.updateMatrixPalette = function () {
            var pe;
    ...
    var BatchManager = /** @class */ (function () {
                    ...
                    if (node.model.isStatic) {
                        ...
                        for (i = 0; i < drawCalls.length; i++) {
                            if (!drawCalls[i]._staticSource)
                                continue;
                            if (!nodeMeshInstances.includes(drawCalls[i]._staticSource))
                                continue;
            ...
            if (!groupIds) {
                ...
                if (groupIds === this._dirtyGroups) {
                    ...
                    for (i = 0; i < this._dirtyGroups.length; i++) {
                        if (!groupIds.includes(this._dirtyGroups[i]))
         ...
         */
        BatchManager.prototype.update = function (_a) {
            var _aabb = _a._aabb, origMeshInstances = _a.origMeshInstances, meshInstance = _a.meshInstance;
            _aabb.copy(origMeshInstances[0].aabb);
            for (var i = 0; i < origMeshInstances.length; i++) {
                if (i > 0)
                    _aabb.add(origMeshInstances[i].aabb); // this is the slowest part
            }
         ...
         */
        BatchManager.prototype.clone = function (_a, clonedMeshInstances) {
            var dynamic = _a.dynamic, batchGroupId = _a.batchGroupId, meshInstance = _a.meshInstance, origMeshInstances = _a.origMeshInstances;
            var batch2 = new pc.Batch(clonedMeshInstances, dynamic, batchGroupId);
