pc.extend(pc, (function () {
    ...
    var _uniformsCollide = function (layers, chain, count, _a) {
        var definition = _a.definition;
        var uniforms = _collectUniforms(definition.fshader);
     ...
     */
    var PostEffectPass = /** @class */ (function () {
        function PostEffectPass(app, options) {
            ...
            this.layer = new pc.Layer({
                ...
                onPostRender: function () {
                    ...
                    if (self.srcRenderTarget)
                        return; // don't do anything else if this effect was not reading backbuffer RT
            ...
            if (!_constInput) {
                ...
                app.on("prerender", function () {
                    ...
                    if (app.scene.layers._dirty) {
                        ...
                        var collisions = void 0, k = void 0;
                        for (i = 0; i < layers.length; i++) {
                            ...
                            if (layers[i].isPostEffect && (iterator === 0 || (layers[i].unmodifiedUvs && layers[i].shader && !_uniformsCollide(layers, _postEffectChain, iterator, layers[i].shader)))) {
                                ...
                                if (i === layers.length - 1)
                                    breakChain = true; // this is the last layer
                            }
                            else {
                                if (iterator > 0) {
                    ...
                    */
                    for (i = 0; i < layers.length; i++) {
                        if (layers[i].isPostEffect && ((!layers[i].postEffect.srcRenderTarget && !layers[i]._postEffectCombined) ||
                            ...
                            if (rtId === 1)
                                _backbufferRt2Used = true; // 2nd backbuffer RT used
                            ...
                            if (layers[i].postEffect.shader && !layers[i].renderTarget)
                                rtId = 1 - rtId; // pingpong RT if posteffect both reads/writes backbuffer
                        }
