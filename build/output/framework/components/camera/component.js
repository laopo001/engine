pc.extend(pc, (function () {
    ...
    pc.extend(CameraComponent.prototype, {
        ...
        onSetClearColor: function (name, oldValue, _a) {
            var data = _a.data;
        ...
        onSetRect: function (name, oldValue, _a) {
            var data = _a.data;
            this.data.camera.setRect(data[0], data[1], data[2], data[3]);
        },
        onSetScissorRect: function (name, oldValue, _a) {
            var data = _a.data;
            ...
            if (display) {
                ...
                if (display.capabilities.canPresent) {
                    ...
                    display.requestPresent(function (err) {
                        if (!err) {
                            self_1.vrDisplay = display;
                            // camera component uses internal 'before' event
                            // this means display nulled before anyone other
                            // code gets to update
                            self_1.vrDisplay.once('beforepresentchange', function (_a) {
                                var presenting = _a.presenting;
                                if (!presenting) {
                                    self_1.vrDisplay = null;
                                }
                            });
                        }
                        callback(err);
