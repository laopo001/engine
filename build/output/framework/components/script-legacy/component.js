    ...
    pc.extend(ScriptLegacyComponent.prototype, {
        ...
        onSetScripts: function (name, oldValue, newValue) {
            if (!this.system._inTools || this.runInTools) {
                ...
                var urls = scripts.map(function (_a) {
                    var url = _a.url;
                    return url;
                });
        ...
        _loadScripts: function (urls) {
            ...
            urls.forEach(function (url) {
                ...
                _this.system.app.loader.load(_url, "script", function (err, ScriptType) {
                    count--;
                    if (!err) {
                        // ScriptType is null if the script is not a PlayCanvas script
                        if (ScriptType && _this.entity.script) {
                            if (!_this.entity.script.instances[ScriptType._pcScriptName]) {
                                var instance = new ScriptType(_this.entity);
                                _this.system._preRegisterInstance(_this.entity, _unprefixed, ScriptType._pcScriptName, instance);
                            }
                        }
                    }
                    else {
                    ...
                    if (count === 0) {
                        _this.data.areScriptsLoaded = true;
                        // We only need to initialize after preloading is complete
                        // During preloading all scripts are initialized after everything is loaded
                        if (!_this.system.preloading) {
                            _this.system.onInitialize(_this.entity);
                            _this.system.onPostInitialize(_this.entity);
                        }
                    }
                });
            });
