...
(function (pc) {
    ...
    if (/(windows|mac os|linux|cros)/i.test(ua))
        pc.platform.desktop = true;
    if (/xbox/i.test(ua))
        pc.platform.xbox = true;
    if (/(windows phone|iemobile|wpdesktop)/i.test(ua)) {
        pc.platform.desktop = false;
        pc.platform.mobile = true;
        pc.platform.windows = true;
    }
    else if (/android/i.test(ua)) {
        pc.platform.desktop = false;
        pc.platform.mobile = true;
        pc.platform.android = true;
    }
    else if (/ip([ao]d|hone)/i.test(ua)) {
        pc.platform.desktop = false;
        pc.platform.mobile = true;
        pc.platform.ios = true;
    }
    if (navigator.isCocoonJS)
        pc.platform.cocoonjs = true;
    pc.platform.touch = 'ontouchstart' in window;
    pc.platform.gamepads = 'getGamepads' in navigator;
})(pc || (pc = {}));
