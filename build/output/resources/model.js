pc.extend(pc, (function () {
     ...
     */
    var ModelHandler = /** @class */ (function () {
        function ModelHandler(device) {
            this._device = device;
            this._parsers = [];
            this.addParser(new pc.JsonModelParser(this._device), function (url, data) { return pc.path.getExtension(url) === '.json'; });
        }
        /**
         * @function
         ...
         */
        ModelHandler.prototype.open = function (url, data) {
        ...
        ModelHandler.prototype.patch = function (asset, assets) {
            if (!asset.resource)
                return;
            var data = asset.data;
            asset.resource.meshInstances.forEach(function (meshInstance, i) {
                if (data.mapping) {
                    var handleMaterial_1 = function (asset) {
                        if (asset.resource) {
                            meshInstance.material = asset.resource;
                        }
                        else {
                            asset.once('load', handleMaterial_1);
                            assets.load(asset);
                        }
                        asset.once('remove', function (_a) {
                    ...
                    if (id !== undefined) { // id mapping
                        if (!id) {
                            ...
                            if (material) {
                                handleMaterial_1(material);
                            }
                            else {
                                assets.once("add:" + id, handleMaterial_1);
                            }
                        }
                    }
                    else if (url) {
                        ...
                        if (material) {
                            ...
                            assets.once("add:url:" + path, handleMaterial_1);
                        }
                    }
                }
            });
         ...
         */
        ModelHandler.prototype.addParser = function (parser, decider) {
            this._parsers.push({
                parser: parser,
                decider: decider
            });
